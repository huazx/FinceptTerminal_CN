...
QString CrashRecoveryDialog::format_relative(qint64 unix_ms) const {
    ...
    if (min < 60) return min == 1 ? tr("1 minute ago")
                                  : tr("%1 minutes ago").arg(min);
    const qint64 hr = min / 60;
    if (hr < 24) return hr == 1 ? tr("1 hour ago")
                                : tr("%1 hours ago").arg(hr);
...
void CrashRecoveryDialog::on_restore_clicked() {
    ...
    selected_snapshot_id_ = item->data(Qt::UserRole).toLongLong();
    restored_ = true;
    accept();
}

void CrashRecoveryDialog::restore_selected() {
    if (selected_snapshot_id_ < 0 || !ring_) {
        restored_ = false;
        return;
    }

    const qint64 id = selected_snapshot_id_;
    ...
    if (payload.isEmpty()) {
        LOG_WARN(kRecoveryTag, QString("snapshot %1 has empty payload").arg(id));
        restored_ = false;
        return;
    }
