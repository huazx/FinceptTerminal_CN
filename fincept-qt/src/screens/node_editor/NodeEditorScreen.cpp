...
void NodeEditorScreen::on_import_workflow() {
    QString path = QFileDialog::getOpenFileName(this, tr("Import Workflow"), {}, tr("JSON Files (*.json);;All Files (*)"));
    if (path.isEmpty())
        return;
...
void NodeEditorScreen::on_export_workflow() {
    QString path =
        QFileDialog::getSaveFileName(this, tr("Export Workflow"), "workflow.json", tr("JSON Files (*.json);;All Files (*)"));
    if (path.isEmpty())
        return;
...
void NodeEditorScreen::on_load_workflow() {
    ...
    *conn =
        connect(&svc, &WorkflowService::workflows_listed, this, [this, conn](const QVector<WorkflowDef>& workflows) {
            ...
            bool ok = false;
            QString chosen = QInputDialog::getItem(this, tr("Load Workflow"), tr("Select a workflow:"), items, 0, false, &ok);
            if (!ok)
                return;
...
void NodeEditorScreen::on_show_templates() {
    ...
    bool ok = false;
    QString chosen = QInputDialog::getItem(this, tr("Workflow Templates"), tr("Select a template:"), templates, 0, false, &ok);
    if (!ok)
        return;
