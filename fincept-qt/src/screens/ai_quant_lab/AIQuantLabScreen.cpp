...
QWidget* AIQuantLabScreen::build_left_sidebar() {
    ...
    for (int i = 0; i < modules_.size(); ++i) {
        const auto& mod = modules_[i];
        if (mod.category_key != last_cat) {
            last_cat = mod.category_key;
            auto* cat = new QLabel(mod.category, list);
...
QWidget* AIQuantLabScreen::build_right_sidebar() {
    ...
    add_stat(tr("Modules"), QString::number(modules_.size()));
    add_stat(tr("ML Models"), "30+");
    add_stat(tr("RL Algorithms"), "5");
    add_stat(tr("Python Scripts"), "25+");
