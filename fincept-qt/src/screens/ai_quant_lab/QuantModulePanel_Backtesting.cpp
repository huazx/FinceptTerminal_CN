...
void QuantModulePanel::display_backtest_result(const QJsonObject& payload) {
    ...
    QList<KpiCard> kpis = {
        {tr("TOTAL RETURN"),   fmt_pct(total_ret),  fmt_usd(final_val) + tr(" final"),  total_ret >= 0, false},
        {tr("ANN. RETURN"),    fmt_pct(ann_ret),     QString(tr("Vol: %1%")).arg(ann_vol, 0, 'f', 1), ann_ret >= 0, false},
        {tr("SHARPE RATIO"),   QString::number(sharpe, 'f', 3),
                           sharpe >= 1 ? tr("Excellent") : sharpe >= 0.5 ? tr("Good") : tr("Weak"), sharpe >= 0.5, false},
        {tr("MAX DRAWDOWN"),   fmt_pct(max_dd),      QString(tr("Calmar: %1")).arg(calmar, 0, 'f', 3), false, false},
        {tr("WIN RATE"),       QString("%1%").arg(win_rate, 0, 'f', 1), QString(tr("%1 days")).arg(t_days), win_rate >= 50, false},
    ...
    status_label_->setText(QString(tr("Done — %1% return  |  Sharpe %2"))
                               .arg(total_ret, 0, 'f', 2)
                               .arg(sharpe, 0, 'f', 3));
}
