...
VideoPlayerWidget::VideoPlayerWidget(QWidget* parent) : BaseWidget(QCoreApplication::translate("VideoPlayerWidget", "LIVE TV / STREAMS"), parent, ui::colors::AMBER()) {
...
void VideoPlayerWidget::build_channel_list() {
    ...
    channel_header_ = new QLabel(QCoreApplication::translate("VideoPlayerWidget", "FINANCIAL TV"));
    ...
    // Custom URL
    custom_header_ = new QLabel(QCoreApplication::translate("VideoPlayerWidget", "CUSTOM STREAM"));
    ...
    url_input_->setPlaceholderText(QCoreApplication::translate("VideoPlayerWidget", "YouTube URL, HLS (.m3u8), MP4, or direct stream..."));
    connect(url_input_, &QLineEdit::returnPressed, this, &VideoPlayerWidget::play_custom_url);
    irl->addWidget(url_input_, 1);

    play_btn_ = new QPushButton(QCoreApplication::translate("VideoPlayerWidget", "PLAY"));
    ...
    helper_label_ = new QLabel(QCoreApplication::translate("VideoPlayerWidget", "YouTube streams resolved via yt-dlp and played inline."));
...
void VideoPlayerWidget::build_player_view() {
    ...
    status_label_placeholder_ =
        new QLabel(QCoreApplication::translate("VideoPlayerWidget", "Qt Multimedia not available.\nBuild with Qt6 Multimedia for inline playback."));
    ...
    stop_btn_ = new QPushButton(QString(QChar(0x25A0)) + QCoreApplication::translate("VideoPlayerWidget", " STOP"));
...
void VideoPlayerWidget::play_custom_url() {
    ...
    if (!url.startsWith("http://") && !url.startsWith("https://"))
        url.prepend("https://");
    play_url(url, QCoreApplication::translate("VideoPlayerWidget", "Custom Stream"));
}
...
void VideoPlayerWidget::play_url(const QString& url, const QString& title) {
    ...
    set_title(QCoreApplication::translate("VideoPlayerWidget", "LIVE TV — ") + title.toUpper());
...
void VideoPlayerWidget::refresh_data() {
    ...
    play_url(current_url_, current_title_.isEmpty() ? QCoreApplication::translate("VideoPlayerWidget", "Custom Stream") : current_title_);
}
...
void VideoPlayerWidget::resolve_youtube_and_play(const QString& youtube_url, const QString& title) {
    ...
    if (ytdlp_program.isEmpty()) {
        set_loading(false);
        status_label_->setText(QCoreApplication::translate("VideoPlayerWidget", "yt-dlp not found. Bundle yt-dlp.exe next to FinceptTerminal.exe."));
        status_label_->show();
...
void VideoPlayerWidget::on_ytdlp_finished(int exit_code, QProcess::ExitStatus /*status*/) {
    ...
    if (exit_code != 0) {
        ...
        set_loading(false);
        status_label_->setText(QCoreApplication::translate("VideoPlayerWidget", "yt-dlp error: ") + (err.isEmpty() ? QCoreApplication::translate("VideoPlayerWidget", "Unknown error") : err.left(120)));
        status_label_->show();
    ...
    if (stream_url.isEmpty()) {
        set_loading(false);
        status_label_->setText(QCoreApplication::translate("VideoPlayerWidget", "Could not extract stream URL."));
        status_label_->show();
        return;
    }
...
void VideoPlayerWidget::on_ytdlp_error(QProcess::ProcessError /*error*/) {
    ...
    set_loading(false);
    status_label_->setText(QCoreApplication::translate("VideoPlayerWidget", "Failed to start yt-dlp: ") + (err.isEmpty() ? QCoreApplication::translate("VideoPlayerWidget", "Unknown error") : err.left(90)));
    status_label_->show();
...
void VideoPlayerWidget::stop_playback() {
    ...
    set_loading(false);
    status_label_->hide();
    set_title(QCoreApplication::translate("VideoPlayerWidget", "LIVE TV / STREAMS"));
    stack_->setCurrentIndex(0);
}

void VideoPlayerWidget::set_loading(bool loading) {
    if (loading) {
        status_label_->setText(QCoreApplication::translate("VideoPlayerWidget", "Resolving stream via yt-dlp..."));
        status_label_->show();
    } else {
        status_label_->hide();
    }
