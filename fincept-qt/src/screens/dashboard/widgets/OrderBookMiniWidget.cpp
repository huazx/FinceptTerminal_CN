...
OrderBookMiniWidget::OrderBookMiniWidget(const QJsonObject& cfg, QWidget* parent)
    : BaseWidget(QCoreApplication::translate("OrderBookMiniWidget", "WORKING ORDERS"), parent) {
    ...
    table_->setHorizontalHeaderLabels({QCoreApplication::translate("OrderBookMiniWidget", "Symbol"), QCoreApplication::translate("OrderBookMiniWidget", "Side"), QCoreApplication::translate("OrderBookMiniWidget", "Qty"), QCoreApplication::translate("OrderBookMiniWidget", "Price"), QCoreApplication::translate("OrderBookMiniWidget", "Status"), QString()});
...
void OrderBookMiniWidget::apply_config(const QJsonObject& cfg) {
    ...
    if (!account_id_.isEmpty()) {
        ...
        header_hint_->setText(QCoreApplication::translate("OrderBookMiniWidget", "No active account — click gear to configure"));
    }
...
void OrderBookMiniWidget::populate(const QVector<trading::BrokerOrderInfo>& rows) {
    ...
    for (int i = 0; i < working.size(); ++i) {
        ...
        auto* price = new QTableWidgetItem(o.price > 0 ? QString::number(o.price, 'f', 2) : QCoreApplication::translate("OrderBookMiniWidget", "MKT"));
        ...
        cancel_btn->setToolTip(QCoreApplication::translate("OrderBookMiniWidget", "Cancel order %1").arg(o.order_id));
...
void OrderBookMiniWidget::cancel_order(const QString& order_id) {
    if (account_id_.isEmpty() || order_id.isEmpty())
        return;
    if (QMessageBox::question(this, QCoreApplication::translate("OrderBookMiniWidget", "Cancel Order"),
                               QCoreApplication::translate("OrderBookMiniWidget", "Cancel order %1?").arg(order_id),
...
QDialog* OrderBookMiniWidget::make_config_dialog(QWidget* parent) {
    ...
    dlg->setWindowTitle(QCoreApplication::translate("OrderBookMiniWidget", "Configure — Working Orders"));
    ...
    form->addRow(QCoreApplication::translate("OrderBookMiniWidget", "Broker account"), combo);
