...
void ChatSessionPanel::update_stats(const ChatStats& stats) {
    stats_lbl_->setText(QString(tr("%1 sessions | %2 messages")).arg(stats.total_sessions).arg(stats.total_messages));
}
...
void ChatSessionPanel::on_delete_clicked() {
    ...
    if (QMessageBox::question(this, tr("Delete Conversation"), QString(tr("Delete \"%1\"?")).arg(confirm_name),
...
void ChatSessionPanel::on_export_clicked() {
    ...
    ChatModeService::instance().export_sessions(uuids, [this, path](bool ok, QJsonArray payload, QString err) {
        ...
        file.close();
        QMessageBox::information(this, tr("Export"), QString(tr("Exported %1 conversations.")).arg(payload.size()));
    });
}
