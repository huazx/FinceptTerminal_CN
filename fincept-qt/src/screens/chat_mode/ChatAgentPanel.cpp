...
void ChatAgentPanel::refresh_tasks() {
    ChatModeService::instance().list_tasks([this](bool ok, QVector<AgentTask> tasks, QString err) {
        ...
        task_status_lbl_->setText(QString(tr("%1 task(s)")).arg(tasks_.size()));
        if (tasks_.isEmpty()) {
...
void ChatAgentPanel::refresh_mcp_servers() {
    ChatModeService::instance().list_mcp_servers(
        ...
        mcp_tools_lbl_->setText(QString(tr("%1 server(s) | %2 tools")).arg(mcp_servers_.size()).arg(total_tools));
...
void ChatAgentPanel::on_delete_memory() {
    ...
    if (QMessageBox::question(this, tr("Delete Memory"), QString(tr("Delete \"%1\"?")).arg(key),
...
void ChatAgentPanel::on_delete_mcp_server() {
    ...
    if (QMessageBox::question(this, tr("Remove Server"), QString(tr("Remove \"%1\"?")).arg(name),
                              QMessageBox::Yes | QMessageBox::No) != QMessageBox::Yes)
        return;
