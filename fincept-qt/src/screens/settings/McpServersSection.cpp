...
void McpServersSection::build_ui() {
    ...
    make_tab(tr("Installed Servers"), 0);
    make_tab(tr("Tools"), 1);
    tbbl->addStretch();
    ...
    if (auto* badge = findChild<QLabel*>("McpToolsBadge"))
        badge->setText(tr("%1 internal tools active").arg(n));
}
...
void McpServersSection::on_add_server() {
    ...
    if (r.is_err()) {
        show_status(tr("Failed to save server: %1").arg(QString::fromStdString(r.error())), true);
    } else {
        show_status(tr("Server added: %1").arg(name), false);
        load_servers();
    ...
    for (const auto& cfg : servers) {
        ...
        detail_lbl_->setText(QString("<b style='color:" + QString(ui::colors::TEXT_PRIMARY()) +
                                     ...
                                     .arg(cfg.name, status_str, cfg.command, cfg.args.join(' '),
                                          cfg.category.isEmpty() ? "-" : cfg.category, cfg.auto_start ? tr("Yes") : tr("No")));
        ...
        QMetaObject::invokeMethod(
            ...
            [self, r, id]() {
                if (!self)
                    return;
                if (r.is_err())
                    self->show_status(tr("Start failed: %1").arg(QString::fromStdString(r.error())), true);
                else
